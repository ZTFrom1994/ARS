$(function() {
	$(".td-pass").click(function(){
		...
		$(this).addClass("travel-selected");
		$(this).next().removeClass("no-pass-click");
		$(this).next().removeClass("travel-selected");
		$("#btn_travel_submit").removeAttr("disabled");
	});
	$(".td-no-pass").click(function(){
		$(this).addClass("no-pass-click");
		$(this).addClass("travel-selected");
		$(this).prev().removeClass("pass-click");
		$(this).prev().removeClass("travel-selected");
		$("#btn_travel_submit").removeAttr("disabled");
	});
	$('#table_audit_travel_reimbursement').DataTable( {
		"paging" : true,
		"searching" : true,
		"ordering" : true,
		"info" : true,
	...
	$("#btn_travel_submit").click(function(){
		var str = "";
		for ( var int = 0; int < $(".travel-selected").size(); int++) {
			str += $(".travel-selected").eq(int).attr("title")+",";
		}
		if (str != "") {
			str = str.substring(0, str.length - 1);
		}
		$.ajax({
			url:"reservationProcessAction!travelProcess",
			type:"post",
			data:{
				order:str
			},
